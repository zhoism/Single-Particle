...
- **Gate encoding** — the **4 P1 AMBER gates landed 2026-06-27** ([[Next_Session_Prompt_AMBER_Gate_Encoding]] consumed); remaining: `ntx`↔`irest` restart-coherence gate ([[Next_Session_Prompt_ntx_irest_CoherenceGate]], ready) · GB-radii `mbondi2` fix + flip-fatal ([[Next_Session_Prompt_GB_Radii_Fix]], ready) · `mdin-edit` whitelist (ready). Backlog tracked in [[Gap_Gate_Coverage]] (partially-filled).
