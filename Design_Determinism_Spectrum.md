...
- **Self-evolution as a *headline* feature was evaluated and declined** ([[Research_Hermes_Agent]], 2026-06-27). Hermes Agent sits at this same 4th pole but pushes the **self-rewrite knob** — the agent authors/rewrites its own skills at runtime. The verdict refines this axis into two: (1) *self-mutation* (does the agent rewrite its skills off-git, unreviewed) and (2) *execution-mediation* (does the LLM call chemistry tools itself vs. hand off to a frozen wrapper). Project Prime stays low on both: boundary tolerates LLM flexibility; the engine room stays frozen + gated regardless of how good self-evolution gets — because **gates bound physics, not intent fidelity** (a self-edited or weaker boundary can emit a wrong-but-physically-valid action that every gate passes).
