...
### P1 — ✅ ALL 4 ENCODED 2026-06-27 · GB-radii fix + flip-fatal landed 2026-06-29 (batch fully closed)
...
> 1. **GB radii ↔ igb** → encoded as a **NON-FATAL detector** (`GB_RADII_IGB_MISMATCH` in `validation.gb_radii` + the mmgbsa note; `MMGBSA_IGB` is the single source of truth for the igb written to `mmgbsa.in` AND the check; helpers `prmtop_radius_set` / `gb_radii_check`). **Decision 2026-06-27 (user):** a *fatal* gate would redden every GREEN MM-GBSA run and the proper mbondi2 fix shifts the reported ΔG → the fix is **DEFERRED** to [[Next_Session_Prompt_GB_Radii_Fix]] (flip-to-fatal is a one-line wire-in once the fix lands). Verified: oracle 60/60 (conda py3.11; helpers stdlib-portable under py3.14) + 78/78 real-prmtop regression + a real MMPBSA run stays `ok:true` with ΔG −18.16 **unchanged** (the detector is purely additive). **✅ FIXED + FLIPPED FATAL 2026-06-29** ([[Next_Session_Prompt_GB_Radii_Fix]] consumed; project-prime `61f6a2f`): Route A — `a_mmgbsa` retypes the dry component tops mbondi→mbondi2 via `parmed changeRadii` (`TARGET_RADIUS_SET = IGB_RADIUS_SET[MMGBSA_IGB]`, single source of truth), and a surviving mismatch now reds the suite via `suite_ok`. ΔG re-baselined (mbondi2, user-signed-off): **1L2Y −17.78** (was −18.16), **3HTB −25.85** (was −27.41); old numbers pre-mbondi2-fix. Oracle 73/0 + acceptance Cases 1 & 4 + two adversarial reviews (SOUND-WITH-FIXES). **The P1 batch is now 4/4 fully closed.**
