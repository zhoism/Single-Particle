...
- memory feedback-verify-and-eval + vault Eval_Criteria.md (THE working practice — apply it this session: pre-run decision gate, deterministic gates, adversarial second-AI review per deliverable)
...
Working practice (established 2026-06-10, memory feedback-verify-and-eval): before committing compute, run a PRE-RUN DECISION GATE — batch the key Stage-6 calls (which run's output to profile [1L2Y vs 3HTB], frame policy [representative frame vs per-frame; start single-frame], skill name, wire-into-run_happy_path or keep standalone) into ONE AskUserQuestion checkpoint for sign-off, then proceed autonomously. Define the success criteria up-front from Eval_Criteria.md (skill-build gates) and match a past skill's format. Deterministic gates (test_acceptance.sh) FIRST, then an ADVERSARIAL second-AI review of the built skill (told to find faults vs the criteria, real artifacts in hand) before declaring done.
