...
A user-scope `Stop` hook (`~/.claude/helpers/hook-handler.cjs`, wired in `settings.json`; recorded in [[claude-harness-statusline-hooks]]) is the safety net. A `SessionStart` hook snapshots each repo's `HEAD` + working-tree at session start (keyed by `session_id`); on stop, if **this session** left work that's uncommitted, committed-without-a-`Dev_Log`-entry, or unpushed in either tracked repo, it **blocks the stop once** with a short checklist. It compares against the session-start baseline, so the repo's chronic pre-existing dirt does **not** trigger it.

It can't *write* the updates — only judgment can — but it won't let a session end with the bookkeeping silently undone. It's deliberately conservative and **one-time**: a trivial commit that doesn't warrant the full sync is dismissed by simply stopping again (`stop_hook_active` is honored, so a second stop always proceeds). No hard push block — review-before-push stays a discipline, reinforced by the nudge surfacing unpushed commits.
